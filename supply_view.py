...
@app.route("/supply/list")
def index_sup():
  return render_template("supply/list.html", supplies=supplies)

@app.route("/supply/add", methods=['GET','POST'])      # 추가 (get: 작성화면보기 / post: 작성후 비품 목록으로 이동)
def add():
...
@app.route("/supply/read")           # 비품번호를 이용해 항목 출력
def read():
...
@app.route("/supply/up", methods=['POST'])       # 갯수 증가
def up():
...
@app.route("/supply/down", methods=['POST'])     # 갯수 감소
def down():
  sno = int(request.form.get('sno'))
  for s in supplies:
...
@app.route("/supply/delete", methods=['POST'])   # 삭제
def delete():
  sno = int(request.form.get('sno'))
  for s in supplies:
    if s['sno']==sno:
